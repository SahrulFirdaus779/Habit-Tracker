...
def load_all_user_data(db_file):
    ...
    for sheet_name in xls.sheet_names:
        try:
            ...
            df_user['User'] = sheet_name # Tambah kolom identitas user
...
def save_data_to_excel(df, username):
    ...
    # Gunakan mode 'a' (append) dengan 'replace' untuk sheet yang ada, atau 'w' (write) untuk file baru
...
with st.sidebar:
    ...
    st.info("Setiap nama akan dibuatkan sheet-nya sendiri. Pastikan nama yang dimasukkan selalu sama.")
    
    today_date = datetime.now().date()
    # Menentukan awal pekan (Senin)
...
tab1, tab2, tab3 = st.tabs(["📊 Progress Pekan Ini", "🗓️ Progress Bulan Ini", "🏆 Leaderboard"])

with tab1:
    st.header(f"Laporan Progress Pribadi Pekan Ini ({username})")
    if df.empty:
...
with tab2:
    st.header(f"Laporan Progress Pribadi Bulan Ini ({username})")
    if df.empty:
        st.warning("Belum ada data untuk ditampilkan. Silakan isi jurnal harian Anda terlebih dahulu.")
    else:
        today_date = datetime.now().date()
